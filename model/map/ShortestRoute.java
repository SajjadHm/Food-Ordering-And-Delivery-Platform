...
public class ShortestRoute {
    ...
    public static void main(String[] args) {
        ...
        try {
            File graphFile = new File("./model/map/graph.txt");
        ...
        int[] shortestRouteFound = findShortestPath(graph, source, destination);

        if (shortestRouteFound.length == 0) {
            System.out.println("No path found from node " + sourcePlusOne + " to node " + destinationPlusOne);
        } else {
            System.out.print("Shortest path from node " + sourcePlusOne + " to node " + destinationPlusOne + ": ");
